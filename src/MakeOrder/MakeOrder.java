...
public class MakeOrder {
    ...
    public static void order(Login user) {
        ...
        scanner.nextLine();
        // กรอกที่อยู่ที่จะส่ง
        if(Member.members.get(user.getId()).getAddress()!=null) {
            System.out.println("Do you want to sent at " + Member.members.get(user.getId()).getAddress() + "? if Yes press 1,No press 2");
            int defaultYN = scanner.nextInt();

            if (defaultYN == 1) {
                // ดึงไปเก็บข้อมูลในอัลเรย์
                addressVector.add(Member.members.get(user.getId()).getAddress());//1
            }
            scanner.nextLine();
             if (defaultYN == 2) {
                ...
                if (defaultAddress == 1) {
                    // ดึงไปเก็บข้อมูลในอัลเรย์
                    Member.setAddress(user.getId(),address);
                }
       ...
       if(Member.members.get(user.getId()).getAddress()==null) {
           //System.out.print("address = "+user.getAddress(user.getName()));
            System.out.println("Please fill in the address you want to ship to.");
            String address = scanner.nextLine();
            System.out.println("Want to set it as a default sending address? if Yes press 1,No press 2 ");
            int defaultAddress = scanner.nextInt();
