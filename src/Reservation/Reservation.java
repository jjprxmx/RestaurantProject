...
public class Reservation {
    ...
    public void makeReservation(Login user) {
        ...
        do {
            ...
            if (time.checkDate(day, month, year)) {
        ...
        do {
            ...
            if (time.checkTime(round)) {
                continue; // Continue to the next iteration if there is an error
            }

            checkTable = true;
            break; // Exit the loop if the round is valid
        } while (true);

        Table test = new Table(day, month, year, number, round, countPeople);
        tables.add(test); // **
        ...
        dataReservations.add(new DataReservation(user.getName(), number, round, test.getchooseTable(), countPeople, day,
                month, year)); // String course,String round,int table,int partySize,int day,int month,int year
